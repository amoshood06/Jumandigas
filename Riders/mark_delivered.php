...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['order_id'])) {
    ...
    $sql = "UPDATE orders SET status = 'delivered' WHERE id = :order_id AND rider_id = :rider_id";
    $stmt = $pdo->prepare($sql);

    if ($stmt->execute(['order_id' => $order_id, 'rider_id' => $rider_id])) {
        echo "<script>alert('Order has been marked as delivered!'); window.location.href='rider_dashboard.php';</script>";
    } else {
